...
- **【新功能】日志分析Agent**：集成Elasticsearch自动分析应用日志，识别异常模式并生成AI驱动的智能故障报告。
- **【新功能】监控告警Agent**：对接Prometheus/Grafana实时监控，自动分析指标异常（CPU、内存、错误率等），提供AI智能解决方案推荐。
- **【新功能】智能工单系统**：自动从故障日志和监控告警创建Jira工单，基于历史数据提供AI推荐方案，智能检测相似工单。
...
DevOpsGPT 现已集成三个智能代理模块，增强 DevOps 自动化能力：

### 1. 日志分析Agent
- 集成 Elasticsearch 实现自动日志分析
- 使用 AI 识别异常模式
- 生成全面的故障报告
- 自动分类严重性级别

### 2. 监控告警Agent
- 对接 Prometheus/Grafana 进行实时监控
- 检测指标异常（CPU、内存、错误率等）
- 提供 AI 驱动的解决方案推荐
- 按严重性分类追踪和管理告警
...
**详细的智能代理文档请参阅 [智能代理模块文档](AGENT_MODULES_CN.md) 或 [English Documentation](AGENT_MODULES.md)。**
